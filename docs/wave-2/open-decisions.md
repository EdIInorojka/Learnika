...
## Wave 2 closure disposition

Wave 2 now has a deterministic safety harness, protected homework/media metadata APIs, local-only
upload, mock OCR review and local non-persistent confirmation. These foundations do not resolve the
production decisions below.

For closure of the explicitly approved local/mock foundation, unresolved provider, retention,
legal, curriculum and numeric safety decisions are deferred. They continue to block real-child beta
use and any future slice that depends on them. No decision in this register authorizes provider
activation, generated assistance, production media processing or Wave 3 implementation.
...
| Media retention policy | Local 24-hour metadata default exists; production durations are not approved | Legal basis, data category schedule, operational need, deletion SLA, backup behavior and support-access policy | security-privacy, devops-sre | Real child media beta and verified cleanup closure |
...
| Exact homework UX | Parent metadata/upload/mock-review foundation exists; learner assistance UX is not finalized | Mobile-first prototype, learner usability evidence for no-answer help, attempt-before-hint comprehension and low-confidence confirmation | student-parent-ux, product-program | Student-facing homework assistance release |
...
| Safety evaluation thresholds | Deterministic regression harness exists; numeric release thresholds are not approved | Numeric thresholds for answer leakage, severe math error, supported validation accuracy, unsupported overconfidence, voice correction rate, latency and deletion compliance | qa-evaluation, curriculum-knowledge, ai-vision-math, voice-media | Learner-facing assistance, Wave 3 evidence integration and beta readiness |
...
| Provider environment naming | Mock-only boundaries require no provider environment; real-provider inventory is deferred | Final env inventory, `.env.example` updates, disabled defaults, secret ownership and kill-switch behavior | devops-sre, backend-platform | Real provider implementation |
