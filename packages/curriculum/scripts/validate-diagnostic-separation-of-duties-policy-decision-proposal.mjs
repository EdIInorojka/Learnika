...
export function validateSeparationOfDutiesDecisionProposalSlice6ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length) {
    fail("Changed paths must not contain duplicates.");
...
export function validateSeparationOfDutiesDecisionProposalSlice7ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length) {
    fail("Changed paths must not contain duplicates.");
...
export function validateSeparationOfDutiesDecisionProposalSlice8ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
export function validateSeparationOfDutiesDecisionProposalSlice9ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
function ciChangedPaths({ cwd, env, runGit, readEvent }) {
  ...
  if (
    ...
    return validateSeparationOfDutiesDecisionProposalSlice8ChangedPaths(paths);
  }
  if (
    paths.length === wave6Slice7ChangedPaths.length &&
    paths.every((value) => wave6Slice7ChangedPathSet.has(value))
  ) {
    return validateSeparationOfDutiesDecisionProposalSlice7ChangedPaths(paths);
  }
  if (
    paths.length === wave6Slice6ChangedPaths.length &&
    paths.every((value) => wave6Slice6ChangedPathSet.has(value))
  ) {
    return validateSeparationOfDutiesDecisionProposalSlice6ChangedPaths(paths);
  }
...
export function validateSeparationOfDutiesDecisionProposalWorktreeScope(
  ...
  if (
    ...
    paths.length === wave6Slice12ChangedPaths.length &&
    new Set(paths).size === paths.length &&
    paths.every((value) => wave6Slice12ChangedPathSet.has(String(value).replaceAll("\\", "/")))
  ) {
    return paths;
  }
  if (
