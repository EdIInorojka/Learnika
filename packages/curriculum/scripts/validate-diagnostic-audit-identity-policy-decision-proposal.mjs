...
const slice8ChangedPaths = [
  ...slice7ChangedPaths.filter((changedPath) => !slice7PrimaryOnlyPaths.has(changedPath)),
...
export function validateDiagnosticAuditIdentityDecisionProposalSlice8ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
export function validateDiagnosticAuditIdentityDecisionProposalSlice9ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
function validateDiagnosticAuditIdentityDecisionProposalSlice10ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
function validateDiagnosticAuditIdentityDecisionProposalSlice11ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
export function collectDiagnosticAuditIdentityDecisionProposalChangedPaths({
  ...
  if (
    ...
    return validateDiagnosticAuditIdentityDecisionProposalSlice9ChangedPaths(paths);
  if (
    paths.length === slice10ChangedPaths.length &&
    paths.every((value) => slice10ChangedPathSet.has(value))
  )
    return validateDiagnosticAuditIdentityDecisionProposalSlice10ChangedPaths(paths);
  if (
    paths.length === slice8ChangedPaths.length &&
    paths.every((value) => slice8ChangedPathSet.has(value))
  )
    return validateDiagnosticAuditIdentityDecisionProposalSlice8ChangedPaths(paths);
  if (
...
export function validateDiagnosticAuditIdentityDecisionProposalWorktreeScope(
  ...
  if (
    ...
    return validateDiagnosticAuditIdentityDecisionProposalSlice11ChangedPaths(paths);
  }
  if (
    Array.isArray(paths) &&
    paths.length === slice10ChangedPaths.length &&
