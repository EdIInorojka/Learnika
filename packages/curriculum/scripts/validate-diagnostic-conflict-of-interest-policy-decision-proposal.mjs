...
export function validateConflictOfInterestDecisionProposalSlice8ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
function validateConflictOfInterestDecisionProposalSlice10ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
function validateConflictOfInterestDecisionProposalSlice11ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
...
export function collectConflictOfInterestDecisionProposalChangedPaths({
  ...
  if (
    ...
    return validateConflictOfInterestDecisionProposalSlice10ChangedPaths(paths);
  }
  if (
    paths.length === slice8ChangedPaths.length &&
    paths.every((value) => slice8ChangedPathSet.has(value))
  ) {
    return validateConflictOfInterestDecisionProposalSlice8ChangedPaths(paths);
  }
  if (
...
export function validateConflictOfInterestDecisionProposalWorktreeScope(
  ...
  if (
    ...
    return validateConflictOfInterestDecisionProposalSlice11ChangedPaths(paths);
  }
  if (
    Array.isArray(paths) &&
    paths.length === slice10ChangedPaths.length &&
