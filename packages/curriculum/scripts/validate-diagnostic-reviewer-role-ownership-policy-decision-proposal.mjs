...
const wave6Slice6ChangedPaths = [
  ...wave6Slice5ChangedPaths.filter((changedPath) => !slice5PrimaryOnlyPaths.has(changedPath)),
...
const wave6Slice6ChangedPathSet = new Set(wave6Slice6ChangedPaths);
const slice6PrimaryOnlyPaths = new Set([
  "docs/wave-6/diagnostic-audit-identity-policy-decision-proposal.md",
  "docs/wave-6/slice-6-implementation-note.md",
  "packages/curriculum/diagnostic-audit-identity-policy-decision-proposal/grade-7-9-math.audit-identity-policy-decision-proposal.v1.json",
]);
const wave6Slice7ChangedPaths = [
  ...wave6Slice6ChangedPaths.filter((changedPath) => !slice6PrimaryOnlyPaths.has(changedPath)),
...
const wave6Slice7ChangedPathSet = new Set(wave6Slice7ChangedPaths);
const slice7PrimaryOnlyPaths = new Set([
  "docs/wave-6/diagnostic-evidence-storage-retention-policy-decision-proposal.md",
  "docs/wave-6/slice-7-implementation-note.md",
  "packages/curriculum/diagnostic-evidence-storage-retention-policy-decision-proposal/grade-7-9-math.evidence-storage-retention-policy-decision-proposal.v1.json",
]);
const wave6Slice8ChangedPaths = [
  ...wave6Slice7ChangedPaths.filter((changedPath) => !slice7PrimaryOnlyPaths.has(changedPath)),
...
const wave6Slice8ChangedPathSet = new Set(wave6Slice8ChangedPaths);
const wave6Slice8PrimaryOnlyPaths = new Set([
  "docs/wave-6/diagnostic-production-approval-authority-policy-decision-proposal.md",
  "docs/wave-6/slice-8-implementation-note.md",
  "packages/curriculum/diagnostic-production-approval-authority-policy-decision-proposal/grade-7-9-math.production-approval-authority-policy-decision-proposal.v1.json",
]);
const wave6Slice9PrimaryOnlyPaths = new Set([
...
const wave6Slice9ChangedPaths = [
  ...wave6Slice8ChangedPaths.filter((changedPath) => !wave6Slice8PrimaryOnlyPaths.has(changedPath)),
  ...wave6Slice9PrimaryOnlyPaths,
...
export function validateReviewerRoleOwnershipDecisionProposalSlice6ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  const unexpected = normalized.filter((value) => !wave6Slice6ChangedPathSet.has(value));
  if (unexpected.length > 0) fail(`Wave 6 Slice 6 out-of-scope path changed: ${unexpected[0]}`);
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
  if (normalized.length !== wave6Slice6ChangedPaths.length)
    fail(`Wave 6 Slice 6 requires exactly ${wave6Slice6ChangedPaths.length} changed paths.`);
  return normalized;
}

export function validateReviewerRoleOwnershipDecisionProposalSlice7ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  const unexpected = normalized.filter((value) => !wave6Slice7ChangedPathSet.has(value));
  if (unexpected.length > 0) fail(`Wave 6 Slice 7 out-of-scope path changed: ${unexpected[0]}`);
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
  if (normalized.length !== wave6Slice7ChangedPaths.length)
    fail(`Wave 6 Slice 7 requires exactly ${wave6Slice7ChangedPaths.length} changed paths.`);
  return normalized;
}

export function validateReviewerRoleOwnershipDecisionProposalSlice8ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  const unexpected = normalized.filter((value) => !wave6Slice8ChangedPathSet.has(value));
  if (unexpected.length > 0) fail(`Wave 6 Slice 8 out-of-scope path changed: ${unexpected[0]}`);
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
  if (normalized.length !== wave6Slice8ChangedPaths.length)
    fail(`Wave 6 Slice 8 requires exactly ${wave6Slice8ChangedPaths.length} changed paths.`);
  return normalized;
}
export function validateReviewerRoleOwnershipDecisionProposalSlice9ChangedPaths(paths) {
  if (!Array.isArray(paths)) fail("Changed paths must be an array.");
  const normalized = paths.map((value) => String(value).replaceAll("\\", "/"));
  if (new Set(normalized).size !== normalized.length)
    fail("Changed paths must not contain duplicates.");
  const unexpected = normalized.filter((value) => !wave6Slice9ChangedPathSet.has(value));
  if (unexpected.length > 0) fail(`Wave 6 Slice 9 out-of-scope path changed: ${unexpected[0]}`);
  if (normalized.length !== wave6Slice9ChangedPaths.length)
...
export function validateReviewerRoleOwnershipDecisionProposalWorktreeScope(
  ...
  if (
    ...
    normalized.every((value) => wave6Slice9ChangedPathSet.has(value))
  ) {
    return validateReviewerRoleOwnershipDecisionProposalSlice9ChangedPaths(normalized);
  }
  if (
    Array.isArray(normalized) &&
    normalized.length === wave6Slice8ChangedPaths.length &&
    normalized.every((value) => wave6Slice8ChangedPathSet.has(value))
  ) {
    return validateReviewerRoleOwnershipDecisionProposalSlice8ChangedPaths(normalized);
  }
  if (
    Array.isArray(normalized) &&
    normalized.length === wave6Slice7ChangedPaths.length &&
    normalized.every((value) => wave6Slice7ChangedPathSet.has(value))
  ) {
    return validateReviewerRoleOwnershipDecisionProposalSlice7ChangedPaths(normalized);
  }
  if (
    Array.isArray(normalized) &&
    normalized.length === wave6Slice6ChangedPaths.length &&
    normalized.every((value) => wave6Slice6ChangedPathSet.has(value))
  ) {
    return validateReviewerRoleOwnershipDecisionProposalSlice6ChangedPaths(normalized);
  }
  if (
    Array.isArray(normalized) &&
...
function ciChangedPaths({ cwd, env, runGit, readEvent }) {
  ...
  if (
    ...
    return validateReviewerRoleOwnershipDecisionProposalSlice7ChangedPaths(paths);
  if (
  ...
  if (
    paths.length === wave6Slice6ChangedPaths.length &&
    paths.every((value) => wave6Slice6ChangedPathSet.has(value))
  )
    return validateReviewerRoleOwnershipDecisionProposalSlice6ChangedPaths(paths);
  if (
    paths.length === wave6Slice4ChangedPaths.length &&
