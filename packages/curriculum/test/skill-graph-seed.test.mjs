...
test("scope guard permits only the exact Pre-Wave 7 Slice 33 import preview worktree", () => {
  const slice33Paths = [...preWave7Slice33ChangedPaths];

  assert.equal(slice33Paths.length, 10);
  assert.equal(slice33Paths.length, preWave7Slice33ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice33Paths), slice33Paths);
  assert.equal(slice33Paths.includes("apps/web/app/school-demo/import-preview/page.tsx"), true);
  assert.equal(slice33Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice33Paths,
        "apps/api/src/school-demo/school-demo-import.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 34 assignment preview worktree", () => {
  const slice34Paths = [...preWave7Slice34ChangedPaths];

  assert.equal(slice34Paths.length, 10);
  assert.equal(slice34Paths.length, preWave7Slice34ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice34Paths), slice34Paths);
  assert.equal(slice34Paths.includes("apps/web/app/school-demo/assignment-preview/page.tsx"), true);
  assert.equal(slice34Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice34Paths,
        "apps/api/src/school-demo/school-assignment.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 35 delivery rehearsal worktree", () => {
  const slice35Paths = [...preWave7Slice35ChangedPaths];

  assert.equal(slice35Paths.length, 10);
  assert.equal(slice35Paths.length, preWave7Slice35ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice35Paths), slice35Paths);
  assert.equal(slice35Paths.includes("apps/web/app/school-demo/delivery-preview/page.tsx"), true);
  assert.equal(slice35Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice35Paths,
        "apps/api/src/school-demo/school-delivery.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 36 student preview worktree", () => {
  const slice36Paths = [...preWave7Slice36ChangedPaths];

  assert.equal(slice36Paths.length, 10);
  assert.equal(slice36Paths.length, preWave7Slice36ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice36Paths), slice36Paths);
  assert.equal(slice36Paths.includes("apps/web/app/school-demo/student-preview/page.tsx"), true);
  assert.equal(slice36Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice36Paths,
        "apps/api/src/school-demo/school-student-delivery.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 37 review queue worktree", () => {
  const slice37Paths = [...preWave7Slice37ChangedPaths];

  assert.equal(slice37Paths.length, 10);
  assert.equal(slice37Paths.length, preWave7Slice37ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice37Paths), slice37Paths);
  assert.equal(slice37Paths.includes("apps/web/app/school-demo/review-queue/page.tsx"), true);
  assert.equal(slice37Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice37Paths,
        "apps/api/src/school-demo/school-review-queue.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 38 review rubric worktree", () => {
  const slice38Paths = [...preWave7Slice38ChangedPaths];

  assert.equal(slice38Paths.length, 10);
  assert.equal(slice38Paths.length, preWave7Slice38ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice38Paths), slice38Paths);
  assert.equal(slice38Paths.includes("apps/web/app/school-demo/review-rubric/page.tsx"), true);
  assert.equal(slice38Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice38Paths,
        "apps/api/src/school-demo/school-review-rubric.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 39 class analytics worktree", () => {
  const slice39Paths = [...preWave7Slice39ChangedPaths];

  assert.equal(slice39Paths.length, 10);
  assert.equal(slice39Paths.length, preWave7Slice39ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice39Paths), slice39Paths);
  assert.equal(slice39Paths.includes("apps/web/app/school-demo/analytics/page.tsx"), true);
  assert.equal(slice39Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice39Paths,
        "apps/api/src/school-demo/school-class-analytics.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 40 teacher dashboard worktree", () => {
  const slice40Paths = [...preWave7Slice40ChangedPaths];

  assert.equal(slice40Paths.length, 10);
  assert.equal(slice40Paths.length, preWave7Slice40ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice40Paths), slice40Paths);
  assert.equal(slice40Paths.includes("apps/web/app/school-demo/teacher-dashboard/page.tsx"), true);
  assert.equal(slice40Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice40Paths,
        "apps/api/src/school-demo/school-teacher-dashboard.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 41 print pack worktree", () => {
  const slice41Paths = [...preWave7Slice41ChangedPaths];

  assert.equal(slice41Paths.length, 10);
  assert.equal(slice41Paths.length, preWave7Slice41ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice41Paths), slice41Paths);
  assert.equal(slice41Paths.includes("apps/web/app/school-demo/print-pack/page.tsx"), true);
  assert.equal(slice41Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice41Paths,
        "apps/api/src/school-demo/school-print-pack.controller.ts",
      ]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 42 school assignment draft foundation worktree", () => {
  const slice42Paths = [...preWave7Slice42ChangedPaths];

  assert.equal(slice42Paths.length, 22);
  assert.equal(slice42Paths.length, preWave7Slice42ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice42Paths), slice42Paths);
  assert.equal(slice42Paths.includes("apps/api/prisma/schema.prisma"), true);
  assert.equal(
    slice42Paths.includes(
      "apps/api/prisma/migrations/20260811120000_school_assignment_draft_foundation/migration.sql",
    ),
    true,
  );
  assert.equal(
    slice42Paths.includes("apps/api/src/school-assignments/school-assignment-draft.service.ts"),
    true,
  );
  assert.throws(
    () =>
...
test("scope guard permits only the exact Pre-Wave 7 Slice 44 assignment draft detail worktree", () => {
  const slice44Paths = [...preWave7Slice44ChangedPaths];

  assert.equal(slice44Paths.length, 10);
  assert.equal(slice44Paths.length, preWave7Slice44ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice44Paths), slice44Paths);
  assert.equal(
    slice44Paths.includes("apps/web/app/school-demo/assignment-drafts/[assignmentCode]/page.tsx"),
    true,
  );
  assert.equal(slice44Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice44Paths,
  ...
  assert.throws(
    () => validateSkillGraphChangedPaths([...slice44Paths, "apps/api/prisma/schema.prisma"]),
    /Runtime or out-of-scope path changed/,
  );
});

test("scope guard permits only the exact Pre-Wave 7 Slice 45 assignment planning worktree", () => {
  const slice45Paths = [...preWave7Slice45ChangedPaths];

  assert.equal(slice45Paths.length, 10);
  assert.equal(slice45Paths.length, preWave7Slice45ChangedPaths.size);
  assert.deepEqual(validateSkillGraphChangedPaths(slice45Paths), slice45Paths);
  assert.equal(
    slice45Paths.includes("apps/web/app/school-demo/assignment-planning/page.tsx"),
    true,
  );
  assert.equal(slice45Paths.includes("apps/web/lib/school-demo-view.ts"), true);
  assert.throws(
    () =>
      validateSkillGraphChangedPaths([
        ...slice45Paths,
